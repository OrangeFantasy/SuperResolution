...
class RepRFN(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, feature_nums=48, sr_scale=4,
                 deploy=False):
...
class RepRFNLoss(torch.nn.Module):
    ...
    def forward(self, target: Tensor, input: Tensor):
        ...
        loss = 0.9 * pixel_loss + 0.1 * fft_loss
