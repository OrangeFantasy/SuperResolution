...
class CAMixerSR(nn.Module):
    def __init__(self, n_block=[4,4,6,6], n_group=4, n_colors=3, n_feats=60, sr_scale=4, ratio=0.5, tile=None):
        super().__init__()
        self.n_feats = n_feats
        ...
        self.tail = nn.Sequential(
            nn.Conv2d(n_feats, n_colors*(sr_scale**2), 3, 1, 1),
...
class CAMixerSRLoss(nn.Module):
    def forward(self, target, output, other_output):
...
if __name__ == '__main__':
    ...
    net = CAMixerSR(sr_scale=4, n_feats=60, ratio=0.5)
    net.eval()
