...
class SuperSampler(nn.Module):
    def __init__(self, frame_ch: int, buffer_ch: int, sr_scale: int, n_history: int):
        ...
        self.conv_out = nn.Sequential(
            nn.Conv2d(_UP_CHS[0] + _DW_CHS[0], frame_ch * sr_scale**2, kernel_size=3, stride=1, padding=1),
            nn.PixelShuffle(sr_scale),
            nn.Conv2d(frame_ch, frame_ch, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True)
        )
